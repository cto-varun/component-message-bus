...
const MessageBus = {
    send: (subject, data) => {
        globalSubject.next({ topic: subject, data });
    },
    request: (requestSub, data, ReplySubject = AsyncSubject) => {
        const replySubject = new ReplySubject();
        MessageBus.send(requestSub, { ...data, replySub: replySubject });
        return replySubject;
    },
        ...
        if (subscriptions[subscriptionId] === undefined) {
            ...
            if (topic.includes('*')) {
                subscription = globalSubject
                    ...
                    .subscribe((e) =>
                        listener(subscriptionId, e.topic, e.data, closure, customData)
                    );
            } else {
                subscription = globalSubject
